...
public class Exercise2Test extends ClassicOnlineStore {
    ...
    public void descSortByAge() {
         ...
         */
        Comparator<Integer> descOrder = (c1,c2) -> c2 - c1;
    ...
    public void top3RichCustomer() {
         ...
         */
        Comparator<Customer> descOrder = (c1,c2) -> c2.getBudget() - c1.getBudget();
        Stream<String> top3RichCustomerStream = customerList.stream().sorted(descOrder).map(Customer::getName).limit(3);
    ...
    public void itemsCustomersWantToBuy() {
        ...
        Stream<String> itemStream = customerList.stream().flatMap(getItemStream).map(c->c.getName());
