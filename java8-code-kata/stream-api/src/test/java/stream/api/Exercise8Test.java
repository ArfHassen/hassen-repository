...
public class Exercise8Test extends ClassicOnlineStore {
    ...
    public void itemsNotOnSale() {
         ...
         */
        Stream<String> customerItemNames = customerStream.flatMap(c->c.getWantToBuy().stream()).map(Item::getName);
        Set<String> shopItemNames = shopStream.flatMap(s->s.getItemList().stream()).map(Item::getName).collect(Collectors.toSet());
        List<String> itemListOnSale = null;
        Set<String> itemSetNotOnSale = customerItemNames.filter(it_c -> !shopItemNames.contains(it_c)).collect(Collectors.toSet());
